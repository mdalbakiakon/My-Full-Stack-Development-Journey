...
const Navbar = ({ textColor = "text-white/70" }) => {
  return (
    <nav className={`fixed top-0 w-full py-2 flex justify-between items-center transition-colors duration-350 z-90 ${textColor}`}>
      <div className="relative text-lg flex justify-between items-center w-full max-w-350 mx-auto bg-[#121212]/10 backdrop-blur-2xl rounded-full px-5 py-2.5">
        ...
        <button className="flex justify-center items-center gap-1.5 cursor-pointer">
          <Send className="w-6 h-6 text-amber-500" /> Contact
        </button>
      </div>
    </nav>
  );
